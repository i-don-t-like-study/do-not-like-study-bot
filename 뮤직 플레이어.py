...
@bot.command()
@commands.has_role('부관리자')
async def 공지(ctx, *, text):
  channel = bot.get_channel(829208129283424276)
...
\> = 봇 명령어
클러야 = 인공지능 대화 명령어
\n>도움 -> 뉴하진윤봇의 모든 명령어를 볼 수 있습니다.
>따라해 [메세지] -> 말을 따라하는 앵무새 뉴하진윤봇! (악용시 처벌)
>나무위키 [찾아볼것] -> 나무위키 링크를 생성합니다
>서버인원수 -> 서버인원수를 확인합니다
\n>들어와 -> 뉴하진윤봇을 자신이 속한 채널로 부릅니다.
\n>나가 -> 뉴하진윤봇을 자신이 속한 채널에서 내보냅니다.
(목록재생에서는 사용할 수 없습니다.)
\n>재생 [노래이름/노래유튜브링크] -> 뉴하진윤이 노래를 검색해 틀어줍니다. + (자동으로 다음 목록에 추가됩니다)
\n>멈춰 -> 현재 재생중인 노래를 끕니다.
